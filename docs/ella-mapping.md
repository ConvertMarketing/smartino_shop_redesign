...
| G3a | **Rail vertical de categorii (desktop)** | **nativ** + JS mic | S | `Header Nav – vertical menu`, tab-ul „Toate categoriile" cu dropdown-ul lui, `Auto show on home page` | **Decizie client (21 sept.):** un singur meniu, cel al Ellei, nu un rail propriu. Dropdown-ul deschis automat pe home dispărea odată cu bara sticky, la primul scroll; `smartino-hero.js` îl mută pe desktop din header în slotul din stânga hero-ului, unde stă în pagină și se derulează cu ea. Pe celelalte pagini se deschide la click. Iconițele: convenția Ellei (`<handle>.png` în Fișiere), mega-menu-urile Ellei pe subcategorii |
...
| H4 | Carusel bestsellers | **nativ** | S | `Product Block` / `Featured Collection` / `Spotlight Products` — coloane separate desktop/tabletă/mobil. Implementat ca `Product Block` în grilă: **5 pe rând pe desktop** (cerința clientului; prototipul are 4), 10 produse, 2 pe rând pe mobil |
...
| H7 | Bloc recenzii (medie + 6 recenzii) | **secțiune nouă** | M | `Customer Review` din Ella e doar un carusel de testimoniale statice: **nu are medie agregată**. Implementat ca `smartino-reviews`: media și numărul din metafield-urile Judge.me ale magazinului (`judgeme.all_reviews_rating` / `all_reviews_count`), cardurile din blocul de aplicație Judge.me „Featured carousel" adăugat în secțiune și re-randat pe designul nostru. **Barele de distribuție pe stele au ieșit din design:** Judge.me nu expune distribuția la nivel de magazin. Vezi nota de sub tabel |
| H8 | Bloc magazine fizice (trust) | **secțiune nouă** | M | `Store Locator`-ul Ella e o pagină cu hartă. `smartino-stores`: antet, hartă Google încărcată doar la click, carduri cu stare „Deschis acum" calculată din program, telefon, „Direcții", loc pentru fotografie, bandă de fapte |
...
| P7 | Grilă 2 col mobil / 5 desktop | **nativ** | S | `View as` / grid density. Implementat: `layout: "5"` pe `main-collection-product-grid` (opțiuni 1–5; clientul a cerut 5 în loc de 4). **Toggle-ul 1↔2 coloane pe mobil = necunoscut** |
